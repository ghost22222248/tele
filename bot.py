...
client = TelegramClient('session_name_47', api_id47,api_hash47)

authorize_client(client)

add_bulk_users_to_group(client,'free_cross_promotions47.csv','megaadvertisement',start=st,mode=2,scrap=True)

st = int(open('last_index.txt','r').read())
client = TelegramClient('session_name, api_id48,api_hash48)

authorize_client(client)

add_bulk_users_to_group(client,'free_cross_promotions2.csv','megaadvertisement',start=st,mode=2,scrap=True)


st = int(open('last_index.txt','r').read())
client = TelegramClient('session_name_19', api_id,api_hash)
